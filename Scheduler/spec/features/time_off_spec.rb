...
describe 'navigation' do
  before do
    ...
    @post1 = TimeOff.create(date: Date.today, rationale: 'Vacation', daily_hours: 0.35, user_id: user.id)
    @user = user
  end
  ...
  describe 'index page' do
    ...
    it 'has a scope of requests to only user' do
      ...
      other_post = TimeOff.create(date: Date.today, rationale: 'Boom', daily_hours: 0.35, user_id: other_user.id)
      visit time_offs_path
  ...
  describe 'creation' do
    ...
    it 'can submit a Time Off Request' do
      ...
      fill_in 'time_off[daily_hours]', with: 0.67
      click_on 'Save'
    ...
    it 'will have an associated User' do
      ...
      fill_in 'time_off[rationale]', with: rationale
      fill_in 'time_off[date]', with: now
      fill_in 'time_off[daily_hours]', with: 0.5
      click_on 'Save'
      expect(User.last.time_offs.last.rationale).to eq(rationale)
      expect(User.last.time_offs.last.daily_hours).to eq(0.5)
    end
  end
  ...
  describe 'edit' do
    before do
      ...
      @other_time_off = TimeOff.create(date: Date.today, rationale: 'Gone', daily_hours: 0.35, user_id: @other_user.id)
